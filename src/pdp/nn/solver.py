...
class SATProblem(object):
    ...
    def setup_problem(self, data_batch, batch_replication):
        ...
        if batch_replication > 1:
            self._replication_mask_tuple = self._compute_batch_replication_map(
                data_batch[1], batch_replication
            )
            (
                self._graph_map,
                self._batch_variable_map,
        ...
        self._vf_mask_tuple = self._compute_variable_function_map(
            self._graph_map,
            self._batch_variable_map,
            self._batch_function_map,
            self._edge_feature,
        )
        self._batch_mask_tuple = self._compute_batch_map(
            self._batch_variable_map, self._batch_function_map
        )
        self._graph_mask_tuple = self._compute_graph_mask(
            self._graph_map,
            self._batch_variable_map,
            self._batch_function_map,
            degree=True,
        )
        self._pos_mask_tuple = self._compute_graph_mask(
            self._graph_map,
            self._batch_variable_map,
            self._batch_function_map,
            (self._edge_feature == 1).squeeze(1).float(),
        )
        self._neg_mask_tuple = self._compute_graph_mask(
            self._graph_map,
            self._batch_variable_map,
            self._batch_function_map,
            (self._edge_feature == -1).squeeze(1).float(),
        )
        self._signed_mask_tuple = self._compute_graph_mask(
            self._graph_map,
            self._batch_variable_map,
            self._batch_function_map,
        ...
        # i = self._graph_map.long()
        # v = self._edge_feature
        # self._clause_mat = torch.sparse.FloatTensor(i, v).to_dense().reshape(self._function_num, self._variable_num)
        # self._edge_mask = sp_aggregators.get_sp_aggregate(self._clause_mat)
    ...
    def _replicate_batch(self, data_batch, batch_replication):
        ...
        ind = (
            torch.arange(batch_replication, dtype=torch.int32, device=self._device)
            .unsqueeze(1)
            .repeat(1, edge_num)
            .view(1, -1)
        )
        graph_map = graph_map.repeat(1, batch_replication) + ind.repeat(
            2, 1
        ) * torch.tensor(
        ...
        batch_variable_map = (
            batch_variable_map.repeat(batch_replication) + ind * batch_size
        )

        ind = (
            torch.arange(batch_replication, dtype=torch.int32, device=self._device)
            .unsqueeze(1)
            .repeat(1, function_num)
            .view(1, -1)
        )
        batch_function_map = (
            batch_function_map.repeat(batch_replication) + ind * batch_size
        )
        ...
        return (
            graph_map,
            batch_variable_map.squeeze(0),
            batch_function_map.squeeze(0),
            edge_feature,
            meta_data,
            label,
        )
    ...
    def _compute_batch_replication_map(self, batch_variable_map, batch_replication):
        batch_size = (batch_variable_map.max() + 1).long().item()
        x_ind = torch.arange(
            batch_size * batch_replication, dtype=torch.int64, device=self._device
        )
        y_ind = torch.arange(batch_size, dtype=torch.int64, device=self._device).repeat(
            batch_replication
        )
        ...
        if self._device.type == "cuda":
            mask = torch.cuda.sparse.FloatTensor(
                ind,
                all_ones,
    ...
    def _compute_variable_function_map(
        self, graph_map, batch_variable_map, batch_function_map, edge_feature
    ):
        edge_num = graph_map.size()[1]
        variable_num = batch_variable_map.size()[0]
        function_num = batch_function_map.size()[0]
        ...
        if self._device.type == "cuda":
            mask = torch.cuda.sparse.FloatTensor(
                graph_map.long(),
                all_ones,
                torch.Size([variable_num, function_num]),
                device=self._device,
            )
            signed_mask = torch.cuda.sparse.FloatTensor(
                graph_map.long(),
            ...
            signed_mask = torch.sparse.FloatTensor(
                graph_map.long(),
                edge_feature.squeeze(1),
                torch.Size([variable_num, function_num]),
                device=self._device,
    ...
    def _compute_batch_map(self, batch_variable_map, batch_function_map):
        ...
        variable_range = torch.arange(
            variable_num, dtype=torch.int64, device=self._device
        )
        function_range = torch.arange(
            function_num, dtype=torch.int64, device=self._device
        )
        batch_size = (batch_variable_map.max() + 1).long().item()
        ...
        if self._device.type == "cuda":
            variable_mask = torch.cuda.sparse.FloatTensor(
                variable_sparse_ind,
                variable_all_ones,
                torch.Size([variable_num, batch_size]),
                device=self._device,
            )
            function_mask = torch.cuda.sparse.FloatTensor(
                function_sparse_ind,
                function_all_ones,
                torch.Size([function_num, batch_size]),
                device=self._device,
            )
        else:
            variable_mask = torch.sparse.FloatTensor(
                variable_sparse_ind,
                variable_all_ones,
                torch.Size([variable_num, batch_size]),
                device=self._device,
            )
            function_mask = torch.sparse.FloatTensor(
                function_sparse_ind,
                function_all_ones,
                torch.Size([function_num, batch_size]),
                device=self._device,
        ...
        return (
            variable_mask,
            variable_mask_transpose,
            function_mask,
            function_mask_transpose,
        )

    def _compute_graph_mask(
        self,
        graph_map,
        batch_variable_map,
        batch_function_map,
        ...
        if self._device.type == "cuda":
            variable_mask = torch.cuda.sparse.FloatTensor(
                variable_sparse_ind,
                edge_values,
                torch.Size([variable_num, edge_num]),
                device=self._device,
            )
            function_mask = torch.cuda.sparse.FloatTensor(
                function_sparse_ind,
                edge_values,
                torch.Size([function_num, edge_num]),
                device=self._device,
            )
        else:
            variable_mask = torch.sparse.FloatTensor(
    ...
    def _compute_adj_list(self):
        ...
        node_list = []
        self.nodes = (
            (self._signed_mask_tuple[0]._indices()[0].to(torch.float) + 1)
            * self._edge_feature.squeeze(1)
        ).to(torch.long)
        for j in range(self._variable_num):
            ...
            functions = self._vf_mask_tuple[3].to(torch.long).to_dense()[
                indices.cpu().numpy(), :
            ][:, j] * (indices + 1)
            node_list.append(functions)
            edge_indices = np.argwhere(
                self._signed_mask_tuple[2].cpu().to_dense()[indices] != 0
            )[1]
            # relations = set([abs(i) - 1 for i in self.nodes[edge_indices].numpy()])
            relations = torch.unique(
                torch.abs(self.nodes[edge_indices]) - 1, sorted=False
            )
    ...
    def _peel(self):
        ...
        while True:
            single_variables = (
                variable_degree == signed_variable_degree.abs()
            ).float() * self._active_variables

            if torch.sum(single_variables) <= 0:
                break

            single_functions = (
                torch.mm(vf_map_transpose, single_variables) > 0
            ).float() * self._active_functions
            degree_delta = torch.mm(vf_map, single_functions) * self._active_variables
            signed_degree_delta = (
                torch.mm(signed_vf_map, single_functions) * self._active_variables
            )
            self._solution[single_variables[:, 0] == 1] = (
                signed_variable_degree[single_variables[:, 0] == 1, 0].sign() + 1
            ) / 2.0
    ...
    def _set_variable_core(self, assignment):
        ...
        # Update the solution
        self._solution[assignment[:, 0].abs() == 1] = (
            assignment[assignment[:, 0].abs() == 1, 0] + 1
        ) / 2.0
    ...
    def _propagate_single_clauses(self):
        ...
        (
            b_variable_mask,
            b_variable_mask_transpose,
            b_function_mask,
            _,
        ) = self._batch_mask_tuple
        ...
        while True:
            ...
            if torch.sum(conflict_variables) > 0:
                ...
                unsat_variables = (
                    torch.mm(b_variable_mask, unsat_examples) * self._active_variables
                )
...
class PropagatorDecimatorSolverBase(nn.Module):
    ...
    def __init__(
        ...
        self._global_step = nn.Parameter(
            torch.tensor([0], dtype=torch.float, device=self._device),
            requires_grad=False,
        )
        self._name = name
    ...
    def forward(
        ...
        if self._propagator is not None and self._decimator is not None:
            propagator_state, decimator_state, loss = self._forward_core(
                init_propagator_state,
            ...
            if loss is not None:
                loss = torch.abs(sat_problem.degree - loss)
        else:
        ...
        if batch_replication > 1:
            prediction, propagator_state, decimator_state = self._deduplicate(
                prediction, propagator_state, decimator_state, sat_problem
            )

        return (
            prediction,
            (propagator_state, decimator_state),
    ...
    def _forward_core(
        ...
        if check_termination is None:
            ...
            active_mask = torch.ones(
                sat_problem._batch_size, 1, dtype=torch.uint8, device=self._device
            )

        for _ in torch.arange(iteration_num, dtype=torch.int32, device=self._device):

            # propagator_state, loss, permutation_state = self._propagator(propagator_state, decimator_state, sat_problem,
            #                                                            is_training, active_mask)
            propagator_state = self._propagator(
                propagator_state, decimator_state, sat_problem, is_training, active_mask
            )
            # print('368', propagator_state[1].shape)
            decimator_state = self._decimator(
                decimator_state, propagator_state, sat_problem, is_training, active_mask
            )

            sat_problem._edge_mask = torch.mm(
                sat_problem._graph_mask_tuple[1], sat_problem._active_variables
            ) * torch.mm(
            ...
            if check_termination is not None:
                ...
                if num_active <= 0:
                    break
        # propagator_state = propagator_state + permutation_state
        # return propagator_state, decimator_state, loss
        return propagator_state, decimator_state, None
    ...
    def _update_solution(self, prediction, sat_problem):
        ...
        if prediction[0] is not None:
            variable_solution = sat_problem._active_variables * prediction[0] + (
                1.0 - sat_problem._active_variables
            ) * sat_problem._solution.unsqueeze(1)
            sat_problem._solution[
                sat_problem._active_variables[:, 0] == 1
    ...
    def _deduplicate(self, prediction, propagator_state, decimator_state, sat_problem):
        ...
        if (
            sat_problem._batch_replication <= 1
            or sat_problem._replication_mask_tuple is None
        ):
            return None, None, None

        assignment = 2 * prediction[0] - 1.0
        energy, _ = self._compute_energy(assignment, sat_problem)
        max_ind = util.sparse_argmax(
            -energy.squeeze(1),
        ...
        variable_prediction = (
            (flag * prediction[0])
            .view(sat_problem._batch_replication, -1)
            .sum(dim=0)
            .unsqueeze(1)
        )
        ...
        for x in propagator_state:
            new_propagator_state += (
                ...
                .view(
                    ...
                    sat_problem._edge_num / sat_problem._batch_replication,
                    -1,
                )
                .sum(dim=0),
            )
        ...
        return (
            (variable_prediction, function_prediction),
            new_propagator_state,
            new_decimator_state,
        )
    ...
    def _local_search(self, prediction, sat_problem, batch_replication):
        ...
        for _ in range(self._local_search_iterations):
            ...
            unsat_variables = (
                torch.mm(sat_problem._vf_mask_tuple[0], unsat_functions)
            ...
            random_ind = util.sparse_argmax(
                unsat_variables.squeeze(1),
                sat_problem._batch_mask_tuple[0],
                device=self._device,
            )

            coin = (
                torch.rand(sat_problem._batch_size, device=self._device) > self._epsilon
            ).long()
    ...
    def _compute_energy_diff(self, assignment, sat_problem):
        ...
        distributed_assignment = torch.mm(
            sat_problem._signed_mask_tuple[1],
            assignment * sat_problem._active_variables,
        )
        aggregated_assignment = torch.mm(
            sat_problem._graph_mask_tuple[2], distributed_assignment
        )
        aggregated_assignment = torch.mm(
            sat_problem._graph_mask_tuple[3], aggregated_assignment
        )
        aggregated_assignment = aggregated_assignment - distributed_assignment

        function_degree = torch.mm(
            sat_problem._graph_mask_tuple[1], sat_problem._active_variables
        )
        ...
        critical_edges = (
            aggregated_assignment == (1 - function_degree)
        ).float() * sat_problem._edge_mask
        delta = torch.mm(
            sat_problem._graph_mask_tuple[0], critical_edges * distributed_assignment
        )
    ...
    def _compute_energy(self, assignment, sat_problem):
        ...
        aggregated_assignment = torch.mm(
            sat_problem._graph_mask_tuple[2], aggregated_assignment
        )
        ...
        ).float() * sat_problem._active_functions
        return (
            torch.mm(sat_problem._batch_mask_tuple[3], unsat_functions),
            unsat_functions,
        )
    ...
    def get_init_state(
        ...
        if self._propagator is None:
            ...
            init_propagator_state = self._propagator.get_init_state(
                graph_map,
                batch_variable_map,
                batch_function_map,
                edge_feature,
        ...
        if self._decimator is None:
            ...
            init_decimator_state = self._decimator.get_init_state(
                graph_map,
                batch_variable_map,
                batch_function_map,
                edge_feature,
    ...
    def __init__(
        ...
        super(NeuralSurveyPropagatorSolver, self).__init__(
            ...
            propagator=pdp_propagate.SurveyPropagator(
                device, decimator_dimension, include_adaptors=True
            ),
            decimator=pdp_decimate.NeuralDecimator(
                device,
                (3, 2),
...
class SurveyPropagatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(
        self, device, name, tolerance, t_max, local_search_iterations=0, epsilon=0.05
    ):
        super(SurveyPropagatorSolver, self).__init__(
            device=device,
            name=name,
            propagator=pdp_propagate.SurveyPropagator(
                device, decimator_dimension=1, include_adaptors=False
            ),
            decimator=pdp_decimate.SequentialDecimator(
                device,
                message_dimension=(3, 1),
                scorer=pdp_predict.SurveyScorer(
                    device, message_dimension=1, include_adaptors=False
                ),
                tolerance=tolerance,
                t_max=t_max,
            ),
...
class WalkSATSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, iteration_num, epsilon=0.05):
        super(WalkSATSolver, self).__init__(
            ...
            decimator=None,
            predictor=pdp_predict.IdentityPredictor(device=device, random_fill=True),
            local_search_iterations=iteration_num,
            epsilon=epsilon,
        )
