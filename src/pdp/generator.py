        ...
        for j in range(self._alpha_resolution):
            ...
            with open(json_output + postfix + ".json", 'w') as f:
                for i in range(size):
                    ...
                    if flag:
                        f.write(str(self._to_json(n, m, graph_map, edge_feature)).replace("'", '"') + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_n', action='store', dest='min_n', type=int, default=100)
    parser.add_argument('--max_n', action='store', dest='max_n', type=int, default=200)
