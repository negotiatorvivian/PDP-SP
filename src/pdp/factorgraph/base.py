...
class FactorGraphTrainerBase:
    ...
    def __init__(
        self, config, has_meta_data, error_dim, loss, evaluator, use_cuda, logger
    ):
        ...
        # Build the network
        self._model_list = [
            self._set_device(model) for model in self._build_graph(self._config)
        ]
    ...
    def _train_epoch(self, train_loader, optimizer):
        train_batch_num = math.ceil(
            len(train_loader.dataset) / self._config["batch_size"]
        )
        ...
        for (j, data) in enumerate(train_loader, 1):  # 下标起始位置为1
            ...
            for i in range(segment_num):
                ...
                (
                    ...
                    variable_num,
                    function_num,
                ) = [self._to_cuda(d[i]) for d in data]
                total_example_num += batch_variable_map.max() + 1
                vf_signed_mask = torch.sparse.FloatTensor(
                    graph_map.long(),
                ...
                if self._config["verbose"]:
                    print(
                        "Training epoch with batch of size {:4d} ({:4d}/{:4d}): {:3d}% complete...".format(
                            batch_variable_map.max().item(),
                            total_example_num % self._config["batch_size"],
                            self._config["batch_size"],
                            int(j * 100.0 / train_batch_num),
                        ),
                        end="\r",
                    )
        ...
        return float(total_loss) / total_example_num  # max(1, len(train_loader))
    ...
    def _train_batch(
        ...
        lambda_value = torch.tensor(
            [self._config["lambda"]], dtype=torch.float32, device=self._device
        )
        ...
        for (i, model) in enumerate(self._model_list):
            ...
            state = _module(model).get_init_state(
                ...
                edge_feature,
                graph_feat,
                self._config["randomized"],
            )

            loss = torch.zeros(1, device=self._device, requires_grad=False)
            # print(loss)

            for t in torch.arange(
                self._config["train_outer_recurrence_num"],
                dtype=torch.int32,
                device=self._device,
            ):
    ...
    def _test_epoch(self, validation_loader, batch_replication):
        ...
        test_batch_num = math.ceil(
            len(validation_loader.dataset) / self._config["batch_size"]
        )
        ...
        with torch.no_grad():
            ...
            for (j, data) in enumerate(validation_loader, 1):
                ...
                for i in range(segment_num):
                    ...
                    total_example_num += (
                        (batch_variable_map.max() + 1).detach().cpu().numpy()
                    )
                    ...
                    if self._config["verbose"]:
                        print(
                            "Testing epoch with batch of size {:4d} ({:4d}/{:4d}): {:3d}% complete...".format(
                                batch_variable_map.max().item(),
                                total_example_num % self._config["batch_size"],
                                self._config["batch_size"],
                                int(j * 100.0 / test_batch_num),
                            ),
                            end="\r",
                        )
    ...
    def _test_batch(
        ...
        for (i, model) in enumerate(self._model_list):
            ...
            prediction, node_adj_lists = model(
                init_state=state,
                graph_map=graph_map,
                batch_variable_map=batch_variable_map,
                batch_function_map=batch_function_map,
    ...
    def _predict_epoch(
        self, validation_loader, post_processor, batch_replication, file
    ):
    ...
    def train(
        ...
        validation_list,
        optimizer,
        last_export_path_base=None,
        best_export_path_base=None,
        metric_index=0,
        load_model=None,
        reset_step=False,
        generator=None,
        ...
        train_loader = FactorGraphDataset.get_loader(
            input_file=train_list[0],
            limit=self._config["train_batch_limit"],
        ...
        validation_loader = FactorGraphDataset.get_loader(
            input_file=validation_list[0],
            limit=self._config["test_batch_limit"],
            hidden_dim=self._config["hidden_dim"],
            batch_size=self._config["batch_size"],
            shuffle=False,
            num_workers=self._num_cores,
            ...
            for epoch in range(self._config["epoch_num"]):
                ...
                if self._config["verbose"]:
                    ...
                    for (i, model) in enumerate(self._model_list):
                        ...
                        message += (
                            "Step {:d}: {:s} error={:s}, {:s} loss={:5.5f} |".format(
                                _module(model)._global_step.int()[0],
                                name,
                    ...
                    self._logger.info(
                        "Rep {:2d}, Epoch {:2d}: {:s}".format(
                            rep + 1, epoch + 1, message
                        )
                    )
    ...
    def test(self, test_list, import_path_base=None, batch_replication=1):
        ...
        if isinstance(test_list, list):
            ...
            test_files = [
                os.path.join(test_list, f)
                for f in os.listdir(test_list)
                if os.path.isfile(os.path.join(test_list, f))
                and f[-5:].lower() == ".json"
            ]
