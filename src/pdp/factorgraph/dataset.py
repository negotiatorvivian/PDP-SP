...
class DynamicBatchDivider(object):
    ...
    def divide(
        ...
        if (self.limit // (max(edge_num) * self.hidden_dim)) >= batch_size:
            ...
            indices = sorted(
                range(len(edge_num)), reverse=True, key=lambda k: edge_num[k]
            )
            ...
            while i < batch_size:
                allowed_batch_size = self.limit // (
                    sorted_edge_num[i] * self.hidden_dim
                )
...
class FactorGraphDataset(data.Dataset):
    ...
    def dag_collate_fn(self, input_data):
        ...
        for i in range(segment_num):
            ...
            edge_feature_batch += [
                torch.from_numpy(
                    np.expand_dims(np.concatenate(edge_feature[i]), 1)
                ).float()
            ]

            # Create the label batch
            label_batch += [
                torch.from_numpy(np.expand_dims(np.array(label[i]), 1)).float()
            ]
