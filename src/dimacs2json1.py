...
class CompactDimacs:
    ...
    def __init__(self, dimacs_file, output, propagate):
        ...
        use_var = max(np.where(np.sum(np.abs(self._clause_mat), 0) > 0)[0])
        if use_var + 1 != var_num:
            print(dimacs_file)
    ...
    def to_json(self):
        ...
        if self.edge_feature is None:
            return [[var_num, clause_num], list((ind[1] + 1) * self._clause_mat[ind]),
                    list(ind[0] + 1)]
        return [[var_num, clause_num], list((ind[1] + 1) * self._clause_mat[ind]),
                 list(ind[0] + 1), list(self._clause_mat[ind] * self.edge_feature)]
